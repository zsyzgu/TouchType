...
class ContactData():
    def __init__(self, id = 0, state = 0, x = 0, y = 0, area = 0, force = 0, major = 0, minor = 0, delta_x = 0, delta_y = 0, delta_force = 0, delta_area = 0):
        self.id = id
        self.state = state
        self.x = x
        self.y = y
...
class FrameData():
    ...
    def output(self, real_contacts = []):
        ...
        for contact in real_contacts:
            cv2.circle(force_map, (int((contact.x * C + 0.5) * MAGNI), int((contact.y * R + 0.5) * MAGNI)), 5, (0, 0, 255), -1)
        cv2.imshow('e', force_map)
        cv2.waitKey(1)